...
def medium_iso_muon_id(mu, ptCut, etaCut, dxyCut, dzCut):
    return (fiducial_muon(mu, ptCut, etaCut, dxyCut, dzCut) and mu.mediumId and mu.pfRelIso03_all<0.35)
...
def medium_iso_electron_id(ele, ptCut, etaCut, dxyCut, dzCut):
    return (fiducial_electron(ele, ptCut, etaCut, dxyCut, dzCut) and ele.mvaSpring16GP_WP90 and ele.pfRelIso03_all<0.35)
...
class preSelection(Module):
    ...
    def analyze(self, event):
        ...
        loose_muons = [ mu for mu in all_muons if loose_muon_id(mu, 8., 2.4, 0.5, 1.) ]
        medium_muons = [mu for mu in all_muons if medium_iso_muon_id(mu, 8., 2.4, 0.5, 1.)]
        loose_muons.sort( key = lambda x: x.pt, reverse=True )

        # Electron selection
        all_electrons  = Collection(event, "Electron")    
        loose_electrons = [ ele for ele in all_electrons if loose_electron_id(ele, 10., 2.5, 0.5, 1.) ]
        medium_electrons = [ele for ele in all_electrons if medium_iso_electron_id(ele, 10., 2.5, 0.5, 1.)]
        ...
        if len(loose_muons) >= 2:
            ...
            event_flag = 3
        elif len(medium_muons) >=1 or len(medium_electrons) >= 1:
            event_flag = 4
            
        if (event_flag not in [1,2,3,4]): 
...
#some doubts!!!
#What about any cut on jet multiplicity?????????
